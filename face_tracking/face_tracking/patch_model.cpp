...
Mat patch_model::calc_response(const cv::Mat &im, const bool sum2one)
{
...
void patch_model::train(const vector<cv::Mat> &images, const Size psize, const float var, const float lambda, const float mu_init, const int nsmaples, const bool visi)
{
...
Mat patch_models::inv_simil(const cv::Mat &S)
{
    Mat Si(2, 3, CV_32F);
...
void patch_models::read(const cv::FileNode &node)
{
    assert(node.type() == FileNode::MAP);
