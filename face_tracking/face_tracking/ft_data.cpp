...
void ft_data::draw_points(cv::Mat &im, const int idx, const bool flipped, const Scalar color, const vector<int>& pts)
{
    if (idx < 0 || idx >= (int)imnames.size())
        return;
    ...
    if (pts.size() == 0)
    {
        ...
        {
            if (!flipped)
                circle(im, points[idx][i], 1, color, CV_AA);
            else
            {
...
void ft_data::draw_sym(cv::Mat &im, const int idx, const bool flipped, const vector<int>& pts)
{
    if (!idx < 0 || idx >= (int)imnames.size())
        return;
...
void ft_data::draw_connect(cv::Mat &im, const int idx, const bool flipped, const Scalar color, const vector<int>& con)
{
    if (idx < 0 || idx >= (int)imnames.size())
        return;
    ...
    if (con.size() == 0)
    {
        for (int i = 0; i < n; ++i)
        {
            ...
            {
                Point2f p { im.cols - 1 - points[idx][symmetry[j]].x, points[idx][symmetry[j]].y };
                Point2f q { im.cols - 1 - points[idx][symmetry[k]].x, points[idx][symmetry[k]].y };
    ...
    else
    {
        ...
        for (int j = 0; j < m; ++j)
        {
            ...
            {
                Point2f p { im.cols - 1 - points[idx][symmetry[k]].x, points[idx][symmetry[k]].y };
                Point2f q { im.cols - 1 - points[idx][symmetry[l]].x, points[idx][symmetry[l]].y };
...
void ft_data::write(cv::FileStorage &fs) const
{
    assert(fs.isOpened());
