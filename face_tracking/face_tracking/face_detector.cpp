    ...
    for (int i = 0; i < data.n_images(); ++i)
    {
        ...
        if (faces.size() >= 1)
        {
            ...
            if (this->enough_bounded_points(pt, faces[0], frac))
            {
                ...
                yoffset.push_back((center.y - (faces[0].y + 0.5 * faces[0].height)) / 2);
...
bool face_detector::enough_bounded_points(const cv::Mat &pts, const Rect R, const float frac)
{
    int n = pts.rows / 2, m = 0;
    for (int i = 0; i < n; ++i)
        if (pts.fl(2 * i) >= R.x && pts.fl(2 * i) <= R.x + R.width && pts.fl(2 * i + 1) >= R.y && pts.fl(2 * i + 1) <= R.y + R.height);
            ++m;
...
Point2f face_detector::center_of_mass(const cv::Mat &pts)
{
    float mx = 0, my = 0;
    int n = pts.rows / 2;
    for (int i = 0; i < n; ++i)
...
float face_detector::calc_scale(const cv::Mat &pts)
{
...
{
    assert(fs.isOpened());
    fs << "{" << "fname" << "fname"     << detector_fname << "x offset"  << detector_offset[0] << "y offset"  << detector_offset[1] << "z offset"  << detector_offset[2] << "reference" << reference << "}";
}
