...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        cout << "usage: ./visualise_patch_model patch_model [-w face_width]"  << endl;
        return 0;
    }
    ...
    for (int i = 0; i < pmodel.n_patches(); ++i)
    {
        ...
        points[i] = Point(scale * pmodel.reference.fl(2 * i) + image_size.width / 2 - P[i].cols / 2, scale * pmodel.reference.fl(2 * i + 1) + image_size.height / 2 - P[i].rows / 2);
